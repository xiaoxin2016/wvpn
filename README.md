...
- **三种 URL 编码模式**（可插拔 `Codec`）：
  ...
  | `subdomain` | `http://a--b-example-com-s.<base>/path` | 每个目标独立浏览器 Origin，隔离最好；需要泛域名 DNS 与泛域名证书。端口编码为 `-p8080` 后缀 |
...
- **管理后台**（`/admin`，改动立即生效，无需重启）：
  - 网关地址：URL 模式（路径 / 加密路径 / 子域名）、网关域名与对外端口，带实时预览；
...
| `-base-domain` / `-public-port` | — | 首次启动时写入的泛域名与对外端口 |
...
URL 模式、网关域名、对外端口都在管理后台"网关地址"里配置，**改完立即生效、无需重启**
（启动参数只在首次运行时写入一次，之后由后台接管）。

子域名模式的部署前提：

1. 一条泛域名解析 `*.app.intra.corp.com` → 网关 IP；
2. 走 HTTPS 时需要该泛域名的证书（`*.app.intra.corp.com` 正好覆盖一层子域）；
3. 后台"网关地址"里填 `app.intra.corp.com`，非默认端口再填对外端口。

地址形态：

```
https://oa.intra.corp.com/portal       → https://oa-intra-corp-com-s.app.intra.corp.com/portal
http://oa.intra.corp.com:8080/portal   → http://oa-intra-corp-com-p8080.app.intra.corp.com/portal
```
...
- **会话 Cookie 的作用域变为 `.app.intra.corp.com`**，因此切换模式后需要重新登录一次。
- **`/login`、`/admin` 等只属于网关自己的主机**；在被代理的子域上，这些路径归目标站点所有，
  不会被网关页面遮蔽。

这也是"赋值式跳转"的最优解：注入脚本知道网关域名，运行期生成的地址同样走子域形式，
...
网关地址配置（模式校验、子域名模式必填域名、端口编码进标签并可往返、超长标签回退路径形式、
运行期切换模式即时生效、身份路由只属于网关主机）、
