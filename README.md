...
**网关的泛域名不要落在被代理站点的注册域之内。** 例如站点在 `*.corp.com`，网关就不要用
`*.wp.corp.com`——那样浏览器里所有 `.corp.com` 的 Cookie 都会发到每个网关子域，既会与网关
下发的同名 Cookie 互相遮蔽（表现为"登录成功后立刻登录态过期"），也会把它们泄露给你经网关访问的
任意站点。用一个独立的注册域，如 `*.wp.example-vpn.com`。详见
[站点兼容性](docs/compatibility.md#网关域名不要落在目标站点的注册域之内)。
