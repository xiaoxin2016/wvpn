...
func (h *Handler) rewriteRequest(pr *httputil.ProxyRequest) {
	...
	//
	// Only what the gateway itself wrote for this browser goes on to the site,
	// under the names the site knows. The rest of the header reached the
...
func (h *Handler) inject(info *reqInfo) string {
	conf := map[string]string{
		"p": PlainPrefix, "t": info.target.String(), "m": h.codec().Name(), "k": CookiePrefix,
	}
